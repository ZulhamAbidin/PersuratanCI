...
public function export_pengajuan_surat() 
{
    // Mendapatkan data pengajuan surat unik beserta data nama dari tabel penduduk
    $this->db->select('DISTINCT(pengajuan_surat.NIK), pengajuan_surat.jenis_surat, pengajuan_surat.tanggal, pengajuan_surat.file, pengajuan_surat.status, penduduk.nama');
    $this->db->from('pengajuan_surat');
    $this->db->join('penduduk', 'pengajuan_surat.NIK = penduduk.NIK');

    $dataPengajuan = $this->db->get()->result_array();
    
    // Nama file CSV yang akan dibuat
    $csvFileName = 'pengajuan_surat.csv';

    // Mengatur header untuk membuat browser mengenali file sebagai file CSV
    header("Content-type: text/csv");
    header("Content-Disposition: attachment; filename=$csvFileName");

    // Membuka file output untuk menulis data CSV
    ...
    // Referensi untuk jenis surat
    $jenisSuratRef = array(
            'SPKK' => 'Kartu Keluarga',
            'SPIPK' => 'Izin Pelataran dan Keramaian',
            'SPKCK' => 'Kelakuan Baik',
            'SKKL' => 'Kelahiran',
            'SKKM' => 'Kematian',
            'SKPD' => 'Pindah Datang',
            'SKKD' => 'Kehilangan Dokumen',
            'SKTM' => 'Telah Menikah',
            'SKPH' => 'Penghasilan',
            'SKB' => 'Berpergian',
            'SKK' => 'Kebersihan',
    ...
    foreach ($dataPengajuan as $row) {
        // Ganti nilai jenis surat dengan referensi jika tersedia, jika tidak, gunakan nilai asli dari tabel sumber
        $jenisSurat = isset($jenisSuratRef[$row['jenis_surat']]) ? $jenisSuratRef[$row['jenis_surat']] : $row['jenis_surat'];
        
        // Memformat tanggal dalam format "d F Y"
        $tanggalFormatted = date("d F Y", strtotime($row['tanggal']));

        fputcsv($output, array(
            $row['NIK'],
            $jenisSurat, // Menggunakan nilai jenis surat yang sudah diganti atau asli
